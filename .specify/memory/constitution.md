...
Modified sections:
  - II. Pipeline Composability: updated embedding file naming convention
    Old: {dataset}.emb-{model}-td.npy / {dataset}.emb-{model}-review.npy
    New: {dataset}.emb-{model_slug}-{modality}.npy (modality: text/visual/review)
  - Technology Stack Constraints: same naming update
...
- 特征编码 → `{dataset}.emb-{model_slug}-{modality}.npy`（modality: text/visual/review）
...
- **数据格式约定**:
  ...
  - 物品嵌入：NumPy NPY（`.emb-{model_slug}-{modality}.npy`，modality: text/visual/review）
